...
import io.github.edufolly.flutterbluetoothserial.WSCmdReplyProc;
import io.github.edufolly.flutterbluetoothserial.WSOutputDataProc;
import com.unitrack.device.rfidreader.ws.handheld.WSProcInterface;
import com.unitrack.device.rfidreader.ws.handheld.data.cmddata.ReadingVersion;
...
public class FlutterBluetoothSerialPlugin implements FlutterPlugin, ActivityAware {
    ...
    // channels.
    private BluetoothConnectionWrapper connections;

    private RFIDReaderInstance rfidReader;
            ...
            public void onReceive(Context context, Intent intent) {
                switch (intent.getAction()) {
                    case BluetoothDevice.ACTION_PAIRING_REQUEST:
                        ...
                        Log.d(TAG, "Pairing request (variant " + pairingVariant + ") incoming from "
                                + device.getAddress());
                                            ...
                                            public void success(Object handlerResult) {
                                                ...
                                                if (handlerResult instanceof String) {
                                                    try {
                                                        ...
                                                        Log.d(TAG, "Trying to set passkey for pairing to "
                                                                + passkeyString);
                                                        ...
                                                        // result.error("bond_error", "Setting passkey for pairing
                                            ...
                                            public void success(Object handlerResult) {
                                                if (handlerResult instanceof Boolean) {
                                                    try {
                                                        ...
                                                        Log.d(TAG, "Trying to set pairing confirmation to " + confirm
                                                                + " (key: " + pairingKey + ")");
                                                        // @WARN `BLUETOOTH_PRIVILEGED` permission required, but might
                                                        // be
                                                        // unavailable for thrid party apps on newer versions of
                                                        // Androids.
                                                    ...
                                                    Log.d(TAG, "Manual passkey confirmation pairing in progress (key: "
                                                            + pairingKey + ")");
                                                    ActivityCompat.startActivity(activity, intent, null);
                                                }
                                                broadcastResult.finish();
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                switch (action) {
                    case BluetoothDevice.ACTION_FOUND:
                        ...
                        // discoveryResult.put("class", deviceClass); // @TODO . it isn't my priority
                        // for now !BluetoothClass!
    ...
    private class BluetoothConnectionWrapper extends BluetoothConnection {
        ...
        public BluetoothConnectionWrapper(BluetoothAdapter adapter) {
            super(adapter);
            // this.id = id;

            readChannel = new EventChannel(messenger, PLUGIN_NAMESPACE + "/read");
        ...
        @Override
        protected void onRead(byte[] buffer, int bytes) {
            rfidReader.recv(buffer, bytes);
            Log.d("FlutterBluePlugin", "---------------------" + bytes);
        ...
        protected void onDisconnected(boolean byRemote) {
            activity.runOnUiThread(() -> {
                if (byRemote) {
                    Log.d(TAG, "onDisconnected by remote (id: " + ")");
                    if (readSink != null) {
                        readSink.endOfStream();
                        readSink = null;
                    }
                } else {
                    Log.d(TAG, "onDisconnected by local (id: " + ")");
                }
            });
        }
    }
        ...
        public void onMethodCall(MethodCall call, Result result) {
            ...
            methodCallDispatching: switch (call.method) {
                ...
                case "getAddress": {
                    ...
                    if (address.equals("02:00:00:00:00:00")) {
                        ...
                        do {
                            ...
                            try {
                                // Requires `LOCAL_MAC_ADDRESS` which could be unavailible for third party
                                // applications...
                            ...
                            try {
                                ...
                                // result.error("hidden_address", "obtaining address using reflection agains
                ...
                case "connect": {
                    ...
                    connections = new BluetoothConnectionWrapper(bluetoothAdapter);

                    if (rfidReader != null) {
                        rfidReader = null;
                    }
                    rfidReader = new RFIDReaderInstance();
                    rfidReader.setReaderType(activeContext);

                    Log.d(TAG, "Connecting to " + address + " (id: " + ")");

                    AsyncTask.execute(() -> {
                        try {
                            connections.connect(address);
                            activity.runOnUiThread(() -> result.success(1));
                        } catch (Exception ex) {
                            activity.runOnUiThread(
                ...
                case "write": {
                    ...
                    BluetoothConnection connection = connections;
                    if (connection == null) {
    ...
    private class RFIDReaderInstance {

        // public static RFIDReaderInstance instance() {
        // if (instance == null)
        // instance = new RFIDReaderInstance();

        // return instance;
        // }

        public void setReaderType(Context context) {
            initWSModule(context);
        }

        public void recv(byte[] buff, int len) {
            if (wsReaderInterface == null)
                return;

            wsReaderInterface.proc(buff, len);

            // when receiving data, update recvDataTime
            recvDataTime = System.currentTimeMillis();

        }

        /*
         * periodically sending heartbeat msg
         * 
         * 1. reader is ws, because ws has no heartbeat msg, just send raading version
         * cmd;
         * 2. reader is self-made, send heartbeat msg
         */
        class RfidReaderTimerTask extends TimerTask {
            ...
            public void run() {
                ...
                Log.d("FlutterBluePlugin", "3333333333333333333333333");
                sendHeartbeatMsg2Ws();

                // if time difference is over 7200000 ms
                // (i.e. if ws rfid reader does not receive data for 1 hour, we think the
                // application should stop running and the reader sheep)
                if ((curTime - recvDataTime) < 3600000) {
                    sendHeartbeatMsg2Ws();
                    // logger.info("Send rading-version cmd to ws handheld rfid reader");
                }

            }
        }

        private RFIDReaderInstance() {
            timer.schedule(new RfidReaderTimerTask(), 10000, HEARTBEAT_PERIOD);

            recvDataTime = System.currentTimeMillis();
        }

        private void initWSModule(Context context) {
            wsReaderInterface = new WSProcInterface();
            cmdReplyProc = new WSCmdReplyProc();
            wsOutputDataProc = new WSOutputDataProc(context);

            wsReaderInterface.setCmdReplyVisitor(cmdReplyProc);
            wsReaderInterface.setOutputDataProc(wsOutputDataProc);
        }

        private void sendHeartbeatMsg2Ws() {
            if (wsReaderInterface == null)
                return;

            ReadingVersion cmd = new ReadingVersion();
            byte[] bytes = wsReaderInterface.encode(cmd);

            AsyncTask.execute(() -> {
                try {
                    if (bytes != null) {
                        Log.d("FlutterBluePlugin", "!!!!!!!!!!!!!!!" + bytes);
                        connections.write(bytes);
                    }
        ...
        // static RFIDReaderInstance instance = null;

        // ws reader related module
        WSProcInterface wsReaderInterface;
        WSCmdReplyProc cmdReplyProc;
        WSOutputDataProc wsOutputDataProc;

        // slefmade reader related module
        // SelfmadeInterface selfmadeInterface;
        // SelfmadeOutputDataProc selfmadeOutputProc;

        private Timer timer = new Timer();

        // data receiving time mark
        private long recvDataTime = 0;

        // 心跳报文周期
        private int HEARTBEAT_PERIOD = 30000;

        // private static Logger logger =
        // Logger.getLogger(RFIDReaderInstance.class.getName());
    }
}
